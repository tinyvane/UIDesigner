...
<p align="center">
  <img src="docs/images/screenshot-editor.png" alt="Dashboard Designer Editor" width="900" />
</p>
...
- **Component Library** — 27 built-in widget types across 10 categories (charts, stats, tables, text, media, maps, buttons, gauges, decorations, utility)
...
| Category | Widgets |
|----------|---------|
| **Charts** | Bar Chart, Line Chart (Gradient Area), Pie Chart, Nested Ring Chart, Gauge |
| **Stats** | Stat Card, Number Flipper, Tech Counter (LCD), Progress Bar, Progress Ring |
| **Tables** | Basic Table, Scrolling Table, Ranking List |
| **Text** | Title, Text Block, Scrolling Text (Marquee) |
| **Media** | Image, Video |
| **Maps** | China Map (with Rotating Halo & Scatter) |
| **Buttons** | Tech Button (Sci-fi Glow), Tech Header (Title Bar) |
...
│   ├── widgets/          # 27 dashboard visualization components
...
- **组件库** — 内置 27 种组件，涵盖 10 大类别（图表、统计、表格、文本、媒体、地图、按钮、仪表盘、装饰、工具）
...
| 类别 | 组件 |
|------|------|
| **图表** | 柱状图、折线图（渐变面积）、饼图、嵌套环形图、仪表盘 |
| **统计** | 统计卡片、数字翻牌器、科技计数器（LCD 风格）、进度条、环形进度 |
| **表格** | 基础表格、滚动表格、排行榜 |
| **文本** | 标题、文本段落、滚动文字（跑马灯） |
| **媒体** | 图片、视频 |
| **地图** | 中国地图（旋转光环 + 散点涟漪） |
...
#### Core Features / 核心功能
- Visual canvas editor with drag-and-drop, multi-select, snap-to-grid, rulers, minimap / 可视化画布编辑器：拖拽、多选、网格吸附、标尺、小地图
- 27 built-in widgets across 10 categories / 27 种内置组件，涵盖 10 大类别
- AI image-to-dashboard: upload screenshot → auto-generate components / AI 截图转大屏：上传截图自动生成组件
- AI chat assistant for layout, colors, design review / AI 聊天助手：布局、配色、设计审查
- Data source system: static JSON, API polling, WebSocket / 数据源系统：静态 JSON、API 轮询、WebSocket
- Export: HTML (static/dynamic), PNG, PDF, embed code / 导出：HTML、PNG、PDF、嵌入代码
- i18n: English + Chinese with full UI coverage / 国际化：英文 + 中文

#### New Widgets / 新增组件
- Tech Button — sci-fi circular button with glow rings / 科技按钮：发光环效果
- Tech Header — title bar with decorative border / 科技标题栏：装饰性底边
- Nested Ring Chart — concentric percentage rings / 嵌套环形图：同心圆百分比
- Tech Counter — LCD-style counter with corner decorations / 科技计数器：LCD 风格 + 四角装饰
- Enhanced Line Chart — gradient area fill / 折线图增强：渐变面积填充
- Enhanced China Map — rotating halo + scatter points / 中国地图增强：旋转光环 + 散点
- Enhanced Pie Chart — tech blue-cyan gradient color scheme / 饼图增强：科技蓝青渐变配色

#### AI Recognition / AI 识别
- Smart prop normalization: direction→horizontal, barColor→color / 智能属性名映射
- Multi-format data parsing: [{name,value}] and {categories,values} / 多格式数据解析
- Image perceptual hash caching for repeat recognition / 图片感知哈希缓存
